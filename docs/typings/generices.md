...
设计泛型的关键目的是为了在成员之间提供有意义的约束，这些成员可以是：
...
在上述代码中存在一个问题，它允许你向队列中添加任何类型的数据，当然，当数据被弹出队列时，也可以为任意类型。在下面的示例中，看起来可以向队列中添加`string` 类型的数据，但是实际上，该用法假定的是只有 `number` 类型会被添加到队列里。
...
class QueueNumber {
  private data = [];
  push = (item: number) => this.data.push(item);  // Error：类型`any`的参数不能赋给类型`never`的参数
...
当然，快速也意味着痛苦。例如当你想创建一个字符串的队列时，你将不得不再次修改相当大的代码。我们真正想要的方法是无论什么类型被推入队列，被推出的类型都与推入类型一样。而当你使用泛型时，这将会很容易实现：
